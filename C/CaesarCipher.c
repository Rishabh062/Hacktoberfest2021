...
//code to do encryption of Cesar
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    ...
    fgets(phrase,100,stdin); //pick up phrase
    
    
    printf("Enter the desired Keyn size\n");
    scanf(" %i",&key); // pick up key
    printf("Type C to Encrypt, or D to Decrypt the entered word/phrase\n");
    scanf(" %c", &C_or_D); 


    if (C_or_D == 'C' || C_or_D == 'c') { //Choose between decrypting and encrypting
    ...
    else if (C_or_D == 'D' || C_or_D == 'd') { //Choose between decrypting and encrypting
